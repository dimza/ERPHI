            ...
            <div class=panel-body>
              <table class="table display" id=datatable>
                ...
                <?PHP 
                  ...
                  $qt= mysql_query("select * from invoice where vis='0' order by id_invoice");
                    ...
                    While($qt2= mysql_fetch_assoc($qt))
                      
                      {
                        ...
                        echo '<td><a href="input_item_invoice.php?invoice='.$qt2["no_invoice"].'" target="_Blank">'.$qt2["no_invoice"].'</a>';
                        ...
                        //echo '<td>'.$cm3;
                        echo '<td><a href="view_do.php?do='.$qt2["no_do"].'" target="_Blank">'.$qt2['no_do'].'</a>';                                              
                        echo '<td>'.$clnt3;
                        echo '<td>'.$qt2['date_invoice']; 
                        //echo '<td>'.$qt2['modified_user'];
                        //echo '<td>'.$qt2['modified_date'];
                        echo '<td><a href="edit_do.php?kd='.$qt2["id_invoice"].'" target="_Blank"><i class="fa-edit"></i></a> | ';                              
                        echo '<a href="sys/engine/hapus_invoice.php?kd='.$qt2["id_invoice"].'&&dt='.$today.'&&pg='.$peg.'&&clt='.$qt2["no_invoice"].'&&swch=1" onclick=\'return confirm("Anda yakin ingin menghapus?")\'><i class="fa-trash"></i></a>';                 
              }
          ?>                    
                <tfoot>
                  <tr>
                    <th><a href="add_invoice.php" target="_Blank"><button type=button class="btn btn-sm btn-primary">ADD INVOICE</button></a>
<!--                <th>
                    <th>
                    <th>
