<?PHP 	ob_start();
	...
	require_once(__DIR__.'/../db_con/db.php'); 
		...
		//status bernilai 11 = on progress
		$stat='20';
	...
	if ($com == "1") {
		...
		$sql= "insert INTO `buying` (id_buy, 	quote_no, 		po_in,		po_out,			supplier_no,	company_no,		currency_no, 	date, 		tax,		remark, 	status,		vis,		create_user, 	create_date, 	modified_user, 		modified_date) 	
						VALUES 	  ('$no', 		'$qts', 		'$poi', 	'$pot',			'$spl',			'$cp',			'$mu', 			'$tgl', 	'$tax2',	'$remk', 	'$stat',	'$null',	'$usr', 		'$date', 		'$usr', 			'$date')";	
		...
		$result2 = mysql_query($sql2) or die(mysql_error());		
 		...
 		// update status quote
		$sql3="UPDATE `quote` SET status='11' WHERE quote_no='$qts'";	
